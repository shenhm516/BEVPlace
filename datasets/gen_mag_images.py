...
if __name__ == "__main__":
    ...
    mag_data = np.loadtxt(args.vel_path +'/P3_output.txt', delimiter=',')
    mag_data = mag_data[np.logical_and(mag_data[:, 7] >=0,  mag_data[:, 7] <14)]
    # mag_data[]
    mag_data = mag_data[np.argsort(mag_data[:, 0]),:]
    # print(len(mag_data), np.linalg.norm(mag_data[:,4:7],axis=1))
    max_intensity = np.max(np.linalg.norm(mag_data[:,4:7],axis=1))
    min_intensity = np.min(np.linalg.norm(mag_data[:,4:7],axis=1))
    ...
    for row in range(len(mag_data)):        
        ...
        if np.array_equal(last_pos, np.zeros([3, 1])): 
            last_pos = cur_pos
            last_time = time
        elif np.linalg.norm(cur_pos-last_pos) > 10 and time!=last_time:
