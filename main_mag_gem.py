...
if __name__ == "__main__":
    ...
    if opt.mode.lower() == 'train':
        ...
        # print(len(global_descs))
        
            recalls_nclt = mag_dataset.evaluateResults(global_descs, test_set)# (q_descs, db_descs, q_dataset, db_dataset)
        ...
        for seq in eval_seq:
            test_set = dataset_loader.InferDataset(seq)   
            # print(len(test_set))
            global_descs = infer(test_set)  #return a very large local feature mat could be very slow
        ...
        mean_precision = np.mean(precision_kitti)
