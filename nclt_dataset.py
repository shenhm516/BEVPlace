...
class InferDataset(data.Dataset):
    def __init__(self, seq, dataset_path = './datasets/bevplace++_dataset/datasets/NCLT/'):
        super().__init__()
...
def evaluateResults(global_descs, datasets):
    ...
    gt_thres = 5
    faiss_index = faiss.IndexFlatL2(global_descs[0].shape[1]) #shm: use the first seq as the database
